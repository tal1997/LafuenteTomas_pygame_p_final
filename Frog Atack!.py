    ...
    def mostrar_tiempo(self, ventana):
        ...
        if minutos == 0 and segundos == 0:
            ...
            if self.player.vidas == 0:
                mostrar_imagen_final(self.player.score.puntaje)
...
class Enemigo(pygame.sprite.Sprite):
    ...
    def loop(self, FPS, bloques, player):
        ...
        if tiempo_actual - self.ultima_actualizacion >= 1:
...
def fondo_game_over(ventana, puntaje):
    ...
    texto_puntaje = font.render("Puntaje: {0}".format(puntaje), True, ROJO)
    texto_rect = texto_puntaje.get_rect(center=(ANCHO_VENTANA // 2, ALTO_VENTANA - 80))
    
    ventana.blit(texto_puntaje, texto_rect)    
    pygame.display.flip()
...
def mostrar_imagen_final(ventana, puntaje):
    fondo = pygame.image.load("recursos/youwin.png").convert_alpha()
...
def main(ventana):
    ...
    while run == True:
        ...
        if len(enemigos) == 0:
            mostrar_imagen_final(ventana, score.obtener_puntaje())                
        ...
        if vida == 0:
            game_over = True
            perdedor = fondo_game_over(ventana, score.obtener_puntaje())
